...
enum cs_type_class {
	...
	CS_TYPE_VOID,
	CS_TYPE_ERROR,
	CS_TYPE_EXPREND,	// Signifies end of expression in some functions
...
//
static cs_type_t cs_eval_type(cs_state_t *state);
...
static cs_type_t cs__eval_type_literal(cs_state_t *state) {
	...
	return (cs_type_t){ .levels[0] = CS_TYPE_EXPREND };
...
static cs_type_t cs__eval_type_factor(cs_state_t *state) {
	const cs_strview_t ident = cs_parse_word(state);

	// Parse a literal
	if (!ident.len) return cs__eval_type_literal(state);
	
	// True/false
	if (cs_strview_eq(ident, CS_STRVIEW("true"))
		|| cs_strview_eq(ident, CS_STRVIEW("false"))) {
		return (cs_type_t){ .levels[0] = CS_TYPE_BOOL };
	}
...
static cs_type_t cs__eval_type_grouping(cs_state_t *state) {
	if (*state->src == '(') {
		state->src++;
		...
		return cs__eval_type_factor(state);
	}
}

static cs_type_t cs__eval_type_postfix(cs_state_t *state) {
	cs_type_t type = cs__eval_type_grouping(state);

	cs_parse_whitespace(state, false);
	if (*state->src == '-' && *(state->src+1) == '>') {
		if (type.levels[0] != CS_TYPE_PTR) {
			cs_error_with_context(state,
			"Expect pointer to struct or enum variant with arrow operator!");
			return (cs_type_t){ .levels[0] = CS_TYPE_ERROR };
		}
		type = cs_remove_level(&type);
		if (type.levels[0] != CS_TYPE_STRUCT) {
			cs_error_with_context(state,
		...
		if (type.levels[0] == CS_TYPE_REF) type = cs_remove_level(&type);
		if (type.levels[0] != CS_TYPE_STRUCT) {
			cs_error_with_context(state,
		...
		if ((type.levels[0] & ~CS_TYPE_MUTMASK) != CS_TYPE_PFN) {
			cs_error_with_context(state,
		...
		if (type.levels[1] & CS_TYPE_PFNMASK) {
			type = state->funcs[type.levels[1] & ~CS_TYPE_PFNMASK].sig.ret;
		} else { // Check for function pointers
			type = state->usertypes[type.levels[1]].p.ret;
		}
		return type;
	} else if (*state->src == '[') {
		if (type.levels[0] != CS_TYPE_SLICE
			&& type.levels[0] != CS_TYPE_ARRPTR
			&& type.levels[0] != CS_TYPE_ARR) {
			cs_error_with_context(state,
		...
		if ((type = cs_remove_level(&type)).levels[0] == CS_TYPE_ERROR) {
			cs_error_with_context(state,
...
static cs_type_t cs__eval_type_builtin(cs_state_t *state) {
	if (cs_expect_keyword(state, CS_STRVIEW("lenof"))) {
		if (*state->src++ != '(') goto paren_error;
		if (cs_eval_type(state).levels[0] == CS_TYPE_ERROR) goto expr_error;
		if (*state->src++ != ')') goto paren_error;
		return (cs_type_t){ .levels[0] = CS_TYPE_USIZE };
	} else if (cs_expect_keyword(state, CS_STRVIEW("typeof"))) {
		if (*state->src++ != '(') goto paren_error;
		const cs_type_t type = cs_eval_type(state);
		if (type.levels[0] == CS_TYPE_ERROR) goto expr_error;
		if (*state->src++ != ')') goto paren_error;
		return type;
		...
		return cs__eval_type_postfix(state);
	}
...
static cs_type_t cs__eval_type_prefix(cs_state_t *state) {
	cs_parse_whitespace(state, false);
	...
	case '+': case '-': case '!': case '~': case '&': case '*':
		break;
	default:
		return cs__eval_type_builtin(state);
	}

	state->src++;
	cs_parse_whitespace(state, false);
	if (cs_isdigit(*state->src)) c = ' ';
	cs_type_t type = cs__eval_type_prefix(state);
	if (c == '*') {
		if (type.levels[0] != CS_TYPE_PTR) {
		...
		if ((type = cs_arithmetic_promotion(&type)).levels[0] == CS_TYPE_ERROR) {
			cs_error_with_context(state,
...
static cs_type_t cs__eval_type_multiplicative(cs_state_t *state, cs_type_t left) {
	cs_parse_whitespace(state, false);
	if (*state->src != '*' && *state->src != '/' && *state->src != '%') {
		return cs__eval_type_prefix(state);
	}

	while (*state->src == '*' || *state->src == '/' || *state->src == '%') {
		state->src++;
		cs_parse_whitespace(state, false);
		cs_type_t right = cs__eval_type_prefix(state);
		if (right.levels[0] == CS_TYPE_EXPREND) goto error;
...
static cs_type_t cs__eval_type_cond(cs_state_t *state) {
	cs_type_t type = { .levels[0] = CS_TYPE_ERROR };
	cs_parse_whitespace(state, false);
	if (!cs_expect_keyword(state, CS_STRVIEW("if"))) {
		return cs__eval_type_assign(state);
	}
	if (cs_eval_type(state).levels[0] == CS_TYPE_ERROR) return type;
	cs_parse_whitespace(state, false);
	if (!cs_expect_keyword(state, CS_STRVIEW("then"))) {
		cs_error_with_context(state, "Expected \"then\"");
		return type;
	}
	cs_type_t ontrue = cs_eval_type(state);
	if (ontrue.levels[0] == CS_TYPE_ERROR) return type;
	cs_parse_whitespace(state, false);
...
static cs_type_t cs_eval_type(cs_state_t *state) {
	cs_parse_whitespace(state, false);
	return cs__eval_type_cond(state);
}
