...
void parser_err(
	...
	while (state->lexer.lvl->curr.ty != tok_semicol
		&& state->lexer.lvl->curr.ty != tok_eof
		&& state->lexer.lvl->curr.ty != tok_error) {
...
static bool parser_eat(parser_t *const state, const tokty_t tokty, const char *const expect_msg) {
	if (state->lexer.lvl->curr.ty == tokty) {
		lexer_next(&state->lexer);
		return true;
	} else {
		const err_t err = (err_t){ .ty = err_expected, .msg = expect_msg, .line = state->lexer.lvl->curr.line, .chr = state->lexer.lvl->curr.chr };
...
static int _parse_cast(parser_t *const state, const int type) {
	...
	const int line = state->lexer.lvl->curr.line, chr = state->lexer.lvl->curr.chr;
	int init = state->lexer.lvl->curr.ty == tok_lbrace ? parse_initializer(state, false) : AST_SENTINAL;
...
static int parse_unary(parser_t *const state) {
	const tok_t tok = state->lexer.lvl->curr;
...
static int parse_call(
	...
	int *last = &params;
	while (state->lexer.lvl->curr.ty != tok_eof && state->lexer.lvl->curr.ty != tok_error) {
		const int param = parse_expression(state, prec_ternary);
		*last = param;
		last = &state->buf[param].next;
		if (state->lexer.lvl->curr.ty == tok_comma) lexer_next(&state->lexer);
		else break;
	}
	parser_eat(state, tok_rparen, "\")\"");
	return parser_add(
...
static int parse_ident(parser_t *const state) {
	const tok_t name = state->lexer.lvl->curr;
	lexer_next(&state->lexer);
	return parser_add(state, (ast_t){
		.ty = ast_ident,
		.tok = name,
...
parser_t parse(const char *src, ast_t *astbuf, size_t buflen) {
	...
	state.tymap = ident_map_init(state.tymap_ents, arrlen(state.tymap_ents));
	lexer_next(&state.lexer);
...
static cvrflags_t parse_cvrflags(parser_t *const state) {
	...
	while (true) {
		switch (state->lexer.lvl->curr.ty) {
...
static int parse_type_not_arr_ptr_func(parser_t *const state) {
	...
	case tok_struct:
		{
			const bool is_struct = state->lexer.lvl->curr.ty == tok_struct;
			lexer_next(&state->lexer);
			const int tyname = parse_ident(state);
			int members = AST_SENTINAL;
			if (state->lexer.lvl->curr.ty == tok_lbrace) { // structure definition
				lexer_next(&state->lexer); // parse members
				int *last = &members;
				while (state->lexer.lvl->curr.ty != tok_rbrace && state->lexer.lvl->curr.ty != tok_eof && state->lexer.lvl->curr.ty != tok_error) {
	...
	case tok_enum:
		{
			...
			if (state->lexer.lvl->curr.ty == tok_lbrace) { // enum definition
				lexer_next(&state->lexer); // parse fields
				int *last = &fields;
				while (state->lexer.lvl->curr.ty != tok_rbrace && state->lexer.lvl->curr.ty != tok_eof && state->lexer.lvl->curr.ty != tok_error) {
					const int line = state->lexer.lvl->curr.line, chr = state->lexer.lvl->curr.chr;
					const tok_t field_name = state->lexer.lvl->curr;
					int value = AST_SENTINAL;

					lexer_next(&state->lexer);
					if (state->lexer.lvl->curr.ty == tok_eq) {
						lexer_next(&state->lexer);
	...
	case tok_ident: // Using an already made typedef
		{
			if (!ident_map_get(&state->tymap, state->lexer.lvl->curr.lit, state->lexer.lvl->curr.len, NULL)) return AST_SENTINAL;
...
construct_pod:
	if ((u > 1 || f > 1 || l > 2)
		...
		|| (i && f)
		|| (l && f)) {
		parser_err(state, (err_t){ .ty = err_unexpected, .msg = "incorrect type spec info", .line = state->lexer.lvl->curr.line, .chr = state->lexer.lvl->curr.chr });
	}
	unitty_t pod;
	switch (bits) {
...
static int parse_initializer(parser_t *const state, const bool allow_desginator) {
	const int line = state->lexer.lvl->curr.line, chr = state->lexer.lvl->curr.chr;
	if ((state->lexer.lvl->curr.ty == tok_dot || state->lexer.lvl->curr.ty == tok_lbrack) && allow_desginator) {
		if (state->lexer.lvl->curr.ty == tok_dot) lexer_next(&state->lexer);
	...
	if (state->lexer.lvl->curr.ty != tok_lbrace) return parse_expression(state, prec_assign);
	lexer_next(&state->lexer);
...
static int _parse_type(parser_t *const state, const bool multi_decl, const bool allow_idents, const int tyspec) {
	...
	while (state->lexer.lvl->curr.ty == tok_star // Check for anything for pointer wise
		|| state->lexer.lvl->curr.ty == tok_lparen
		|| state->lexer.lvl->curr.ty == tok_const
		|| state->lexer.lvl->curr.ty == tok_static
		|| state->lexer.lvl->curr.ty == tok_extern
		|| state->lexer.lvl->curr.ty == tok_restrict
		|| state->lexer.lvl->curr.ty == tok_volatile
		|| state->lexer.lvl->curr.ty == tok_rparen) {
		bparens += state->lexer.lvl->curr.ty == tok_lparen;
		if (state->lexer.lvl->curr.ty == tok_rparen && --bparens <= 0) break;
		before[++btop] = state->lexer.lvl->curr;
		lexer_next(&state->lexer);
	}
	if (state->lexer.lvl->curr.ty == tok_ident) { // Get identifier if we can
		if (!allow_idents) {
			parser_err(state, (err_t){ .ty = err_unexpected, .msg = "unexpected identifier", .line = state->lexer.lvl->curr.line, .chr = state->lexer.lvl->curr.chr });
			return AST_SENTINAL;
		}
		ident = state->lexer.lvl->curr;
		lexer_next(&state->lexer);
	...
	while (true) {
		...
		while (state->lexer.lvl->curr.ty == tok_lbrack
			|| state->lexer.lvl->curr.ty == tok_lparen) {
			if (state->lexer.lvl->curr.ty == tok_lbrack) {
				lexer_next(&state->lexer);
				...
				*parent = curr_node;
				...
				int params = AST_SENTINAL;
				int *last = &params;
				while (state->lexer.lvl->curr.ty != tok_rparen) {
		...
		bool parened = state->lexer.lvl->curr.ty == tok_rparen && btop >= 0;
	...
	if (state->lexer.lvl->curr.ty == tok_eq && allow_idents) {
		lexer_next(&state->lexer);
	...
	if (multi_decl && state->lexer.lvl->curr.ty == tok_comma && allow_idents) {
		lexer_next(&state->lexer);
...
static int parse_type(parser_t *const state, const bool multi_decl, const bool allow_idents) {
	...
	if (state->lexer.lvl->curr.ty == tok_typedef) {
		is_typedef = true;
		lexer_next(&state->lexer);
	}
...
static int parse_compound_stmt(parser_t *const state) {
	...
	int *last = &stmts;
	while (last && state->lexer.lvl->curr.ty != tok_rbrace && state->lexer.lvl->curr.ty != tok_eof && state->lexer.lvl->curr.ty != tok_error) {
...
static void parse_label(parser_t *const state, labelty_t *const ty, int *const label) {
	// get the label
	if (state->lexer.lvl->curr.ty == tok_ident && state->lexer.lvl->peek.ty == tok_colon) {
		*ty = label_norm;
		*label = parse_ident(state);
		lexer_next(&state->lexer);
	} else if (state->lexer.lvl->curr.ty == tok_case) {
		*ty = label_case;
		lexer_next(&state->lexer);
		*label = parse_ident(state);
		parser_eat(state, tok_colon, "\":\"");
	} else if (state->lexer.lvl->curr.ty == tok_default) {
		*ty = label_default;
		*label = AST_SENTINAL;
		lexer_next(&state->lexer);
		parser_eat(state, tok_colon, "\":\"");
...
static int parse_ifelse_stmt(parser_t *const state) {
	...
	if (state->lexer.lvl->curr.ty == tok_else) {
		lexer_next(&state->lexer);
...
static void parse_decl_toplvl(parser_t *const state, int *const first_def, int *const last_def) {
	...
	if (node != AST_SENTINAL
		...
		&& state->lexer.lvl->curr.ty == tok_lbrace) {
...
static int parse_expression(
	...
	const parse_unary_pfn prefix = _rules[state->lexer.lvl->curr.ty].prefix;
	if (!prefix) return AST_SENTINAL;
	int left = prefix(state);

	// Support for (ONLY ternay operator)
	if (state->lexer.lvl->curr.ty == tok_qmark && prec >= prec_ternary) {
		const tok_t tok = state->lexer.lvl->curr;
		lexer_next(&state->lexer);
	...
	} else if (state->lexer.lvl->curr.ty == tok_colon) {
		return left;
	}

	while (prec >= _rules[state->lexer.lvl->curr.ty].prec) {
		const parse_binary_pfn infix = _rules[state->lexer.lvl->curr.ty].infix;
		left = infix(state, _rules[state->lexer.lvl->curr.ty].prec, left);
	}
...
int parser_add(parser_t *const state, const ast_t newnode) {
	if (state->nnodes == state->buflen) {
		parser_err(state, (err_t){ .ty = err_limit, .msg = "max ast nodes reached", .line = state->lexer.lvl->curr.line, .chr = state->lexer.lvl->curr.chr });
		return AST_SENTINAL;
	}
...
static typed_unit_t parse_number(parser_t *const state) {
	...
	const tok_t tok = state->lexer.lvl->curr;
	if (tok.len > 24) {
		parser_err(state, (err_t){ .ty = err_limit, .msg = "exceeded max literal integer size", .line = state->lexer.lvl->curr.line, .chr = state->lexer.lvl->curr.chr });
...
static int parse_string_lit(parser_t *const state) {
	tok_t tok = state->lexer.lvl->curr;
	lexer_next(&state->lexer);
	return parser_add(state, (ast_t){
		.ty = ast_literal,
		.tok = tok,
